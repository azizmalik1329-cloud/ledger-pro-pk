...
export default function Page(){
  ...
  useEffect(()=>{
    if(session?.user.id) void loadApp();
    else resetApp();
  },[session?.user.id]);

  useEffect(()=>{
    if(section==="settings"&&businessId&&role==="owner"&&settingsLoadedFor!==businessId&&!settingsLoading){
      void loadSettingsData(businessId);
    }
  ...
  function resetApp(){
    loadSeq.current+=1;
    setAccountResolved(false);setAccountLoading(false);setAccountError("");
    setIsPlatformAdmin(false);setBusinessId("");setBusiness(null);setMemberships([]);
  ...
  async function loadApp(preferredBusinessId?:string){
    ...
    if(!business) setAccountResolved(false);

    const [{data:membershipRows,error},{data:platformAdmin,error:adminError}]=await Promise.all([
    ...
    const rows=(membershipRows||[]) as unknown as Membership[];
    setMemberships(rows);
    if(!rows.length){
      setBusiness(null);setBusinessId("");setContacts([]);setProducts([]);setTransactions([]);
      setAccountResolved(true);setAccountLoading(false);return;
    }
  ...
  async function loadSettingsData(bid:string,force=false){
    ...
    if(!force&&settingsLoadedFor===bid)return;
    ...
    if(m.error||a.error){toast(m.error?.message||a.error?.message||"Settings data load error");return}
  ...
  async function reloadContacts(){
    ...
    if(error)return toast(error.message);setContacts((data||[]) as Contact[]);
  }
  async function reloadProducts(){
    if(!businessId)return;
    const {data,error}=await supabase.from("products").select("*").eq("business_id",businessId).order("name");
    if(error)return toast(error.message);setProducts((data||[]) as Product[]);
  }
  async function reloadTransactionsAndProducts(){
    ...
    if(t.error||p.error)return toast(t.error?.message||p.error?.message||"Data refresh error");
    setTransactions((t.data||[]) as Tx[]);setProducts((p.data||[]) as Product[]);
  }
  ...
  async function remove(table:"contacts"|"products"|"transactions",id:string,label:string){
    if(table==="contacts"){
      if(!window.confirm(`${label} ko archive karna hai? Purani ledger history mehfooz rahegi.`))return;
      const {error}=await supabase.rpc("archive_contact",{p_business_id:businessId,p_contact_id:id});
      if(error)return toast(error.message);toast("Khata archive ho gaya");void reloadContacts();return;
    }
    if(table==="transactions"){
      const reason=window.prompt(`${label} ko void karna hai. Reason likhein:`);
      if(reason===null)return;
      if(reason.trim().length<3)return toast("Void reason kam az kam 3 characters ka ho");
  ...
  if(!accountResolved)return <div className="splash"><div className="logo">LP</div><p>Business data secure tareeqe se load ho raha hai…</p></div>;
  if(accountError)return <main className="blocked"><div className="logo">LP</div><h1>Data load nahi hua</h1><p>{accountError}</p><button onClick={()=>void loadApp()}>Dobara koshish karein</button><button onClick={()=>supabase.auth.signOut()}>Logout</button></main>;
  ...
  const filteredContacts=contacts.filter(x=>x.is_active&&`${x.name} ${x.phone||""} ${x.email||""}`.toLowerCase().includes(search.toLowerCase()));
  const filteredProducts=products.filter(x=>x.is_active&&`${x.name} ${x.sku||""}`.toLowerCase().includes(search.toLowerCase()));
  const filteredTx=transactions.filter(x=>!x.is_void&&`${x.reference||""} ${txLabel[x.type]} ${contacts.find(c=>c.id===x.contact_id)?.name||""}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <main className={dark?"app dark":"app"} aria-busy={accountLoading}>
    ...
    <section className="workspace">
      ...
      <div className="content">
        ...
        {section==="settings"&&<Settings business={business} role={role} members={members} audit={audit} loading={settingsLoading} reloadBusiness={()=>loadApp(businessId)} reloadSettings={()=>loadSettingsData(businessId,true)}/>} 
      </div>
...
function ContactModal({businessId,item,busy,setBusy,close,done}:{businessId:string;item:Contact|null;busy:boolean;setBusy:(v:boolean)=>void;close:()=>void;done:()=>void}){async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);const f=new FormData(e.currentTarget);const fields={type:f.get("type"),name:String(f.get("name")||"").trim(),phone:String(f.get("phone")||"").trim()||null,email:String(f.get("email")||"").trim()||null,opening_balance:Number(f.get("balance")||0),notes:String(f.get("notes")||"").trim()||null};const query=item?supabase.from("contacts").update(fields).eq("id",item.id).eq("business_id",businessId):supabase.from("contacts").insert({...fields,business_id:businessId});const {error}=await query;setBusy(false);if(error)alert(error.message);else done()}return <Shell title={item?"Khata edit karein":"Naya khata"} close={close}><form onSubmit={submit}><div className="formRow"><label>Type<select name="type" defaultValue={item?.type||"customer"}><option value="customer">Customer</option><option value="supplier">Supplier</option></select></label><label>Naam<input name="name" required minLength={2} defaultValue={item?.name}/></label></div><div className="formRow"><label>Phone<input name="phone" inputMode="tel" defaultValue={item?.phone||""}/></label><label>Email<input name="email" type="email" defaultValue={item?.email||""}/></label></div><label>Opening due<input name="balance" type="number" step="0.01" defaultValue={item?.opening_balance||0}/><small className="fieldHint">Positive = amount due, negative = advance.</small></label><label>Notes<textarea name="notes" defaultValue={item?.notes||""}/></label><ModalActions busy={busy} close={close}/></form></Shell>}
function ProductModal({businessId,item,busy,setBusy,close,done}:{businessId:string;item:Product|null;busy:boolean;setBusy:(v:boolean)=>void;close:()=>void;done:()=>void}){async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);const f=new FormData(e.currentTarget);const basePurchase=Number(f.get("purchase")||0);const fields={name:String(f.get("name")||"").trim(),sku:String(f.get("sku")||"").trim()||null,unit:String(f.get("unit")||"pcs"),sale_price:Number(f.get("sale")||0),base_purchase_price:basePurchase,low_stock_level:Number(f.get("low")||0)};const query=item?supabase.from("products").update(fields).eq("id",item.id).eq("business_id",businessId):supabase.from("products").insert({...fields,business_id:businessId,purchase_price:basePurchase,stock_quantity:Number(f.get("stock")||0)});const {error}=await query;setBusy(false);if(error)alert(error.message);else done()}return <Shell title={item?"Product edit karein":"Naya stock item"} close={close}><form onSubmit={submit}><div className="formRow"><label>Product naam<input name="name" required minLength={2} defaultValue={item?.name}/></label><label>SKU<input name="sku" defaultValue={item?.sku||""}/></label></div><div className="formRow"><label>Unit<select name="unit" defaultValue={item?.unit||"pcs"}><option value="pcs">Pieces</option><option value="kg">KG</option><option value="ctn">Carton</option><option value="ltr">Litre</option></select></label>{item?<label>Current stock<input value={`${item.stock_quantity} ${item.unit}`} disabled readOnly/><small className="fieldHint">Stock change ke liye Adjust button use karein.</small></label>:<label>Opening stock<input name="stock" type="number" step="0.001" min="0" defaultValue={0}/></label>}</div><div className="formRow"><label>Base purchase rate<input name="purchase" type="number" step="0.01" min="0" defaultValue={item?.base_purchase_price??item?.purchase_price??0}/><small className="fieldHint">Purchase history ho to current rate database latest valid purchase se rakhta hai.</small></label><label>Sale rate<input name="sale" type="number" step="0.01" min="0" defaultValue={item?.sale_price||0}/></label></div><label>Low stock alert<input name="low" type="number" step="0.001" min="0" defaultValue={item?.low_stock_level||0}/></label><ModalActions busy={busy} close={close}/></form></Shell>}
function StockAdjustmentModal({businessId,item,busy,setBusy,close,done}:{businessId:string;item:Product;busy:boolean;setBusy:(v:boolean)=>void;close:()=>void;done:()=>void}){async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const delta=Number(f.get("delta")||0),reason=String(f.get("reason")||"").trim();if(delta===0){alert("Adjustment zero nahi ho sakta");return}if(reason.length<3){alert("Reason kam az kam 3 characters ka ho");return}setBusy(true);const {error}=await supabase.rpc("adjust_product_stock",{p_business_id:businessId,p_product_id:item.id,p_delta:delta,p_reason:reason});setBusy(false);if(error)alert(error.message);else done()}return <Shell title={`${item.name} stock adjust karein`} close={close}><form onSubmit={submit}><div className="adjustmentSummary"><small>Current stock</small><b>{item.stock_quantity} {item.unit}</b></div><label>Quantity adjustment<input name="delta" type="number" step="0.001" required placeholder="Example: 5 ya -2"/><small className="fieldHint">Positive = stock add, negative = stock minus.</small></label><label>Reason<textarea name="reason" required minLength={3} placeholder="Damage, physical count correction, opening correction…"/></label><p className="modalNote">Har adjustment audit trail mein user, time, quantity aur reason ke sath save hoti hai.</p><ModalActions busy={busy} close={close}/></form></Shell>}
function TransactionModal({businessId,item,defaultType,contacts,products,busy,setBusy,close,done}:{businessId:string;item:Tx|null;defaultType:Tx["type"];contacts:Contact[];products:Product[];busy:boolean;setBusy:(v:boolean)=>void;close:()=>void;done:()=>void}){const [kind,setKind]=useState<Tx["type"]>(item?.type||defaultType);const inventory=kind==="sale"||kind==="purchase";const availableContacts=contacts.filter(c=>c.is_active||c.id===item?.contact_id);const availableProducts=products.filter(p=>p.is_active||p.id===item?.product_id);async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);const f=new FormData(e.currentTarget),amount=Number(f.get("amount")||0),paid=inventory?Number(f.get("paid")||0):amount,productId=String(f.get("product")||"")||null,quantity=inventory?Number(f.get("quantity")||0):null;if(amount<=0){alert("Total amount zero se zyada hona chahiye");setBusy(false);return}if(inventory&&(!productId||!quantity||quantity<=0)){alert("Product aur quantity zaroor select karein");setBusy(false);return}if(inventory&&(paid<0||paid>amount)){alert("Paid amount total se zyada nahi ho sakta");setBusy(false);return}const fields={contact_id:String(f.get("contact")||"")||null,product_id:inventory?productId:null,quantity,type:kind,reference:String(f.get("reference")||"").trim()||null,amount,paid_amount:paid,transaction_date:String(f.get("date")||today()),notes:String(f.get("notes")||"").trim()||null,status:paid>=amount?"paid":paid>0?"partial":"unpaid"};const query=item?supabase.from("transactions").update(fields).eq("id",item.id).eq("business_id",businessId):supabase.from("transactions").insert({...fields,business_id:businessId});const {error}=await query;setBusy(false);if(error)alert(error.message);else done()}return <Shell title={item?"Transaction edit karein":"Nayi transaction"} close={close}><form onSubmit={submit}><div className="formRow"><label>Type<select name="type" value={kind} onChange={e=>setKind(e.target.value as Tx["type"])}><option value="sale">Farokht</option><option value="purchase">Khareedari</option><option value="payment_in">Payment wasool</option><option value="payment_out">Payment ada</option><option value="expense">Kharcha</option></select></label><label>Customer / Supplier<select name="contact" defaultValue={item?.contact_id||""}><option value="">General / Walk-in</option>{availableContacts.map(c=><option value={c.id} key={c.id}>{c.name}{c.is_active?"":" (Archived)"} — {c.type}</option>)}</select></label></div>{inventory&&<div className="formRow"><label>Product<select name="product" required defaultValue={item?.product_id||""}><option value="">Product select karein</option>{availableProducts.map(p=><option value={p.id} key={p.id}>{p.name} — {p.stock_quantity} {p.unit}</option>)}</select></label><label>Quantity<input name="quantity" type="number" required min="0.001" step="0.001" defaultValue={item?.quantity||1}/></label></div>}<div className="formRow"><label>Reference<input name="reference" defaultValue={item?.reference||""} placeholder="INV-1001"/></label><label>Date<input name="date" type="date" required defaultValue={item?.transaction_date||today()}/></label></div><div className="formRow"><label>Total amount<input name="amount" type="number" min="0.01" step="0.01" required defaultValue={item?.amount||0}/></label>{inventory?<label>Paid amount<input name="paid" type="number" min="0" step="0.01" defaultValue={item?.paid_amount||0}/></label>:<label>Cash amount<input value="Full amount" disabled readOnly/></label>}</div>{kind==="sale"&&<p className="modalNote">Sale ka stock minus aur COGS snapshot database khud karega.</p>}{kind==="purchase"&&<p className="modalNote">Purchase ka stock plus hota hai; edit/void par purchase rate database dobara calculate karta hai.</p>}<label>Notes<textarea name="notes" defaultValue={item?.notes||""}/></label><ModalActions busy={busy} close={close}/></form></Shell>}
